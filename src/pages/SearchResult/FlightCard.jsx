      ...
      <div className="flex flex-col gap-3 p-4 mt-4 bg-white rounded-md font-lexend flight-details">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex flex-col items-center gap-2">
            ...
            <p className="text-xs font-normal text-[#008CFF]">{flight_group[0].no_of_stops === 0 ? "Direct Flight" : `${flight_group[0].no_of_stops} Stop Flight`}</p>
          </div>
                    ...
                    <div className="flex flex-col items-start justify-between w-full gap-3">
                      ...
                      <div className="flex items-center justify-between w-full">
                        <div className="flex flex-col items-start">
                          ...
                          <p className="text-xs font-normal text-[#8592A6]">{formattedDepartureDate}</p>
                          <p className="text-xs font-normal font-lexend text-[#151516]">{firstRoute.origin_airport.city}</p>
                          <p className="text-xs font-normal text-[#8592A6]">{firstRoute.origin_airport.name}</p>
                        </div>
...

                  {
                    ...
                    <>
                      <div className="flex flex-col items-start justify-between w-full gap-3">
                        ...
                        <div className="flex items-center justify-between w-full">
                          <div className="flex flex-col items-start">
                            <p className="text-2xl font-bold text-[#151516]">{moment(firstRoute.departure_time).format('HH:mm')}</p>                          
                            <p className="text-xs font-normal text-[#8592A6]">{moment(firstRoute.departure_time).format('YYYY-MM-DD')}</p>
                            <p className="text-xs font-normal font-lexend text-[#151516]">{firstRoute.origin_airport.city}</p>
                            <p className="text-xs font-normal text-[#8592A6]">{firstRoute.origin_airport.name}</p>
                          </div>

                          <div className="flex flex-col items-center gap-2">
                            <p className="text-xs font-normal text-[#008CFF]">{formatDuration(firstRoute.flight_time)}</p>
                            <div className="relative w-32 h-[3px] bg-[#008CFF]">
                              <div className="absolute z-20 flex items-center justify-between w-full -top-[3px]">
                                <span className="w-2 h-2 rounded-full bg-[#008CFF]"></span>
                          ...
                          <div className="flex flex-col items-center">
                            <p className="text-2xl font-bold text-[#151516]">{moment(firstRoute.arrival_time).format('HH:mm')}</p>                          
                            <p className="text-xs font-normal text-[#8592A6]">{moment(firstRoute.arrival_time).format('YYYY-MM-DD')}</p>
                            <p className="text-xs font-normal font-lexend text-[#151516]">{firstRoute.destination_airport?.city}</p>
                            <p className="text-base font-normal text-[#8592A6]">{firstRoute.destination_airport?.name}</p>
                          </div>
                        </div>
                      </div>
                      <div className="relative border-b-2 border-slate-200">
                        <div className="absolute flex items-center mx-auto text-xs left-[15%] bg-gray-100 px-3 rounded-md py-1 -top-3">
                          <span className="font-semibold text-yellow-500">Change of Airport </span>
                          <span className="flex items-center justify-center"><GoDotFill />Change of Planes<GoDotFill /> {formatDuration(lastRoute.lay_over)} layover in {lastRoute.origin_airport.name}</span> 
                        </div>
                      </div>
                      <div className="flex flex-col items-start justify-between w-full gap-3">
                        {/* first row */}
                        <div className="flex items-center gap-3">
                          <img src={`https://gtrs-airlineimages.s3.ap-southeast-1.amazonaws.com/icon/` + lastRoute.marketing.carrier_logo} className="h-10 rounded-full" alt="" />
                          <div className="flex flex-col gap-1">
                            <p className="text-base text-[#151516] font-normal">{lastRoute.marketing.carrier_name}</p>
                          </div>
                        </div>
                        {/* second row */}
                        <div className="flex items-center justify-between w-full">
                          <div className="flex flex-col items-start">
                            <p className="text-2xl font-bold text-[#151516]">{moment(lastRoute.departure_time).format('HH:mm')}</p>

                            <p className="text-xs font-normal text-[#8592A6]">{moment(lastRoute.departure_time).format('YYYY-MM-DD')}</p>
                            <p className="text-xs font-normal font-lexend text-[#151516]">{lastRoute.origin_airport.city}</p>
                            <p className="text-xs font-normal text-[#8592A6]">{lastRoute.origin_airport.name}</p>
                          </div>

                          <div className="flex flex-col items-center gap-2">
                            <p className="text-xs font-normal text-[#008CFF]">{formatDuration(lastRoute.flight_time)}</p>
                            <div className="relative w-32 h-[3px] bg-[#008CFF]">
                              <div className="absolute z-20 flex items-center justify-between w-full -top-[3px]">
                                <span className="w-2 h-2 rounded-full bg-[#008CFF]"></span>
                          ...
                          <div className="flex flex-col items-center">
                            <p className="text-2xl font-bold text-[#151516]">{moment(lastRoute.arrival_time).format('HH:mm')}</p>
                            <p className="text-xs font-normal text-[#8592A6]">{moment(lastRoute.arrival_time).format('YYYY-MM-DD')}</p>
                            <p className="text-xs font-normal font-lexend text-[#151516]">{lastRoute.destination_airport?.city}</p>
                            <p className="text-base font-normal text-[#8592A6]">{lastRoute.destination_airport?.name}</p>
                          </div>
                        </div>
